    ...
    def getKeparahanVelue(self):
        try:
            ...
            for i in range(0, self.scrapped_news.shape[0]):
                ...
                document_result.append([title, berita, time, what, tanggal_asli, orang, provinsi, kabupaten, kecamatan, death, injury, lost_items, lost_money,rape])

            writer = pd.DataFrame(document_result, columns=[
                                'title', 'description', 'time', 'what', 'when', 'who', 'provinsi', 'kabupaten / kota', 'kecamatan', "mati", 'luka', 'kerugianBarang', 'kerugianUang','pemerkosaan'], index=None)
